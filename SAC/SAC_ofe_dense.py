...
class Actor(nn.Module):
	def __init__(
		...
		self.mean = nn.Linear(output_features_dim, action_dim)
		self.log_std = nn.Linear(output_features_dim, action_dim)
	...
	def forward(self, state_features, deterministic=False, with_logprob=True):
		...
		mu_a = self.mean(features)
		log_std_a = self.log_std(features)
