...
from spinupUtils.user_config import DEFAULT_DATA_DIR, FORCE_DATESTAMP, DEFAULT_SHORTHAND, WAIT_BEFORE_LAUNCH
from spinupUtils.logx import colorize
from spinupUtils.mpi_tools import mpi_fork, msg
...
def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False):
    ...
    If no seed is given and datestamp is false, `output_dir` = data_dir/exp_name

    If a seed is given and datestamp is false, `output_dir1 = data_dir/exp_name/exp_name_s[seed]

    If datestamp is true, amend to `output_dir` = data_dir/YY-MM-DD_exp_name/YY-MM-DD_HH-MM-SS_exp_name_s[seed]
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    There's also a SpinningUp-specific convenience added into executing the thunk: 
    if `env_name` is one of the kwargs passed to call_experiment, 
    it's assumed that the thunk accepts an argument called `env_fn`, 
    and that the `env_fn` should make a gym environment with the given `env_name`. 
    ...
    Args:
        ...
        num_cpu (int): Number of MPI processes to split into. 
                        Also accepts 'auto', which will set up as many procs as there are cpus on the machine.
        data_dir (string): Used in configuring the logger, to decide where to store experiment results. 
                            Note: if left as None, 
                            data_dir will default to `DEFAULT_DATA_DIR` from `spinup/user_config.py`. 
        **kwargs: All kwargs to pass to thunk.

    """
    # Determine number of CPU cores to run on
    num_cpu = psutil.cpu_count(logical=False) if num_cpu=='auto' else num_cpu
    kwargs['seed'] = seed # Send random seed to thunk
