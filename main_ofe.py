"""For running OFENET coupled with DDPG / TD3 / SAC."""
import random
import torch
import gym
import argparse
...
import numpy as np
# ------------------------------
from DDPG import DDPG_ofe
# ------------------------------
from TD3 import TD3_ofe
# ------------------------------
from SAC import SAC_ofe
# -------------------------------
...
# num of layers for building ofenet
def get_target_layer(env_name):
	target_dim_layer = {
		"Ant-v2": 6,
		"HalfCheetah-v2": 8,
		"Walker2d-v2": 6,
	...
	return target_dim_layer[env_name]

if __name__ == "__main__":
	parser = argparse.ArgumentParser()
	parser.add_argument("--policy", default="DDPG_ofe")              # Policy name
	...
	parser.add_argument("--layer_norm", default=True, type=bool)     # For ofenet layer normalization or not
	...
	kwargs_ofenet = {
		...
		"hidden_dim": 40,
		"num_layers": get_target_layer(args.env),
	...
	# Initialize policy
	# ----------------------------------------------
	if args.policy == "DDPG_ofe":
		policy = DDPG_ofe.DDPG_OFE(**kwargs_policy)
	# ----------------------------------------------
	elif args.policy == "TD3_ofe":
		...
		policy = TD3_ofe.TD3_OFE(**kwargs_policy)
	# ----------------------------------------------
	elif args.policy == "SAC_ofe":
		kwargs_policy["alpha"] = args.alpha
		policy = SAC_ofe.SAC_OFE(**kwargs_policy)
	else:
